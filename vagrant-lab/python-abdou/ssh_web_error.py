...
class FTPLogExtractor:
    ...
    def extract_failed_logins(self, log_data):
        # Pattern pour extraire le timestamp, username, et IP
        pattern = re.compile(r'(\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}) \[error\] \d+#\d+: \*\d+ user "([^"]+)" was not found in "[^"]+", client: ([\d\.]+)')
        failed_logins = pattern.findall(log_data)

        # Réorganiser les éléments pour correspondre à l'ordre attendu (username, ip_address, attempt_time)
        formatted_failed_logins = [(user, ip, timestamp) for timestamp, user, ip in failed_logins]
    ...
    def store_in_db(self, failed_logins):
        ...
        try:
            ...
            for login in failed_logins:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM failed_logins_web WHERE username = %s AND ip_address = %s AND attempt_time = %s",
                ...
                if result[0] == 0:
                    ...
                    cursor.execute(
                        "INSERT INTO failed_logins_web (username, ip_address, attempt_time) VALUES (%s, %s, %s)",
                        (username, ip_address, attempt_time)
                    )
                else:
...
def main():
    try:
        with open('ssh_web_error.json', 'r') as f:
            config = json.load(f)
