...
# Seuils d'alerte
CPU_THRESHOLD = 70  # Pourcentage d'utilisation du CPU
RAM_THRESHOLD = 55   # Pourcentage d'utilisation de la RAM
DISK_THRESHOLD = 40  # Pourcentage d'utilisation du disque
...
class SystemStatus:
    def __init__(self, db_config):
        ...
        self.last_alert_time = datetime.now() - timedelta(hours=1)  # Initialiser à 1 heure dans le passé
    ...
    def get_system_status(self, hostname, username, key_file):
        ...
        try:
            ...
            logging.info(f"Statut du serveur {hostname}: CPU: {cpu}, RAM: {ram}, DISK: {disk}")

            # Vérifier les seuils et envoyer des alertes si nécessaire
            current_time = datetime.now()
            alert_sent = False  # Flag pour vérifier si une alerte a été envoyée

            if cpu_usage > CPU_THRESHOLD and (current_time - self.last_alert_time) >= timedelta(hours=1):
                send_alert_email(SMTP_CONFIG, hostname, cpu, ram, disk)
                self.last_alert_time = current_time
                alert_sent = True

            if ram_usage > RAM_THRESHOLD and (current_time - self.last_alert_time) >= timedelta(hours=1):
                send_alert_email(SMTP_CONFIG, hostname, cpu, ram, disk)
                self.last_alert_time = current_time
                alert_sent = True

            if disk_usage > DISK_THRESHOLD and (current_time - self.last_alert_time) >= timedelta(hours=1):
                send_alert_email(SMTP_CONFIG, hostname, cpu, ram, disk)
                self.last_alert_time = current_time
                alert_sent = True

            if not alert_sent:
                logging.info(f"Aucune alerte envoyée pour {hostname}. Utilisations: CPU: {cpu_usage:.2f}%, RAM: {ram_usage:.2f}%, DISK: {disk_usage}%")
